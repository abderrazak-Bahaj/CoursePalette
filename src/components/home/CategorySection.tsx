...
const CategorySection = () => {
  ...
  return (
    <WrapperLoading isLoading={isLoading} skeletonCount={4} useSkeletonLoader={true} skeletonVariant="list" >
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {mappedCategories?.map((category, index) => (
              ...
              >
                <div className={`rounded-lg p-6 ${category.color} h-full course-card-shadow transition-all duration-300 ${hoveredIndex === index ? "transform -translate-y-1" : ""
                  }`}>
                  <div className="flex flex-col h-full">
                    ...
                    <div className="mt-auto">
                      <span className={`flex items-center text-sm font-medium`}>
                        Explore Category
                        <ChevronRight size={16} className={`ml-1 transition-transform ${hoveredIndex === index ? "transform translate-x-1" : ""
                          }`} />
                      </span>
                    </div>
                  </div>
